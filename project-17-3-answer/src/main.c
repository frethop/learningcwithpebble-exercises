...
// Altered for Project 17.1 
// Altered again for Project 17.3
// 
// Mike Jipping, June 2016 and August 2016
...
static ResHandle adverb_handle;
static size_t adverbsize;

static int nouns, verbs, adjectives, adverbs;
static bool stats_display;

enum Grammar {NOUN, VERB, ADJECTIVE, ADVERB, MADLIB}; 

#define NOUN_STATS 1010
#define VERB_STATS 1011
#define ADJECTIVE_STATS 1012
#define ADVERB_STATS 1013
...
char *random_thing(enum Grammar item) { 
    ...
    nouns = persist_read_int(NOUN_STATS);
    verbs = persist_read_int(VERB_STATS);
    adjectives = persist_read_int(ADJECTIVE_STATS);
    adverbs = persist_read_int(ADVERB_STATS);
    ...
    switch (item) {
        ...
        case ADJECTIVE:
            ...
            break;
        case ADVERB:
            position = rand()%adverbsize;
            handle = adverb_handle;
            size = adverbsize;
            adverbs++;
            break;
        case MADLIB:
    ...
    persist_write_int(NOUN_STATS, nouns);
    persist_write_int(VERB_STATS, verbs);
    persist_write_int(ADJECTIVE_STATS, adjectives);
    persist_write_int(ADVERB_STATS, adverbs);
...
void open_madlibs_files() {
   ...
   // Adverb file resource handle
   adverb_handle = resource_get_handle(RESOURCE_ID_ADJECTIVES);
   adverbsize = resource_size(adverb_handle);
...
void display_madlib() {
    ...
    thing = random_thing(ADVERB);
    replace_string(madlibstr, "<ADVERB>", thing);
    free(thing);
