...
// Altered again for Project 17.2
// 
...
void display_madlib() {
    ...
    int numbytes = persist_read_string(SAVE_ADVERB, saved, 30);
    APP_LOG(APP_LOG_LEVEL_INFO, "GOT A SAVED ADVERB: %s at %d bytes", saved, numbytes);
    if (numbytes < 0) 
        thing = random_thing(ADVERB);
    else 
        do {
            thing = random_thing(ADVERB);
        } while (!strcmp(saved, thing));
    replace_string(madlibstr, "<ADVERB>", thing);
    persist_write_string(SAVE_ADVERB, thing);
    free(thing);
