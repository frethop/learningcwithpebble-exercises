//----------------------------------------------------------------------------------
//
// Project 3.1/15.1 from "Learning C with Pebble"
// 
// Mike Jipping, December 2015 and July 2016
...
#ifdef PBL_COLOR
    position_x = window_frame.size.w - ball_radius - 1;
    position_y = ball_radius+1;
    x_velocity = y_velocity = 5;
#else
    position_x = ball_radius+1;
    position_y = ball_radius+1;
    x_velocity = y_velocity = 10;
#endif    
} 
...
#ifdef WRAPAROUND
    if (position_x + ball_radius + 1 < 0) {        
        position_x = window_frame.size.w + ball_radius;
    } else if  (position_x - ball_radius - 1 > window_frame.size.w) {
