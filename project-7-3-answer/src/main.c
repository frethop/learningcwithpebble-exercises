...
bool solve_maze(int x, int y) {
  solved_maze[x][y] = true;  // solve path includes the spot in current position
  
  // If we're at the finish, then we solved the maze!
  if(x == finish_x && y == finish_y)
    return true;
  
  // Try solving by going up
  if(!maze[x][y - 1] && !solved_maze[x][y - 1]) {  // If you can go up and haven't already been there
    solved_maze[x][y - 1] = true;   // solve path incudes spot above current position
    if(solve_maze(x, y - 2))        // move up two spaces and continue solving
      return true;                  // if solved, return true
    solved_maze[x][y - 1] = false;  // we didn't solve the maze, so remove spot above current position
  }
    
  // Try solving by going down
  if(!maze[x][y + 1] && !solved_maze[x][y + 1]) {  // If you can go down and haven't already been there
    solved_maze[x][y + 1] = true;   // solve path includes spot below current position
    if(solve_maze(x, y + 2))        // move down two spaces and continue solving
      return true;                  // if solved, return true
    solved_maze[x][y + 1] = false;  // we didn't solve the maze, so remove spot below current position
  }
    
  // Try solving by going left
  if(!maze[x - 1][y] && !solved_maze[x - 1][y]) {  // If you can go left and haven't already been there
    solved_maze[x - 1][y] = true;   // solve path includes spot left of the current position
    if(solve_maze(x - 2, y))        // move left two spaces and continue solving
      return true;                  // if solved, return true
    solved_maze[x - 1][y] = false;  // we didn't solve the maze, so remove spot to the left of the current position
  }
    
  // Try solving by going right
  if(!maze[x + 1][y] && !solved_maze[x + 1][y]) {  // If you can go right and haven't already been there
    solved_maze[x + 1][y] = true;   // solve path includes spot right of the current position
    if(solve_maze(x + 1, y))        // move right two spaces and continue solving
      return true;                  // if solved, return true
    solved_maze[x + 1][y] = false;  // we didn't solve the maze, so remove spot to the right of the current position
  }

  // Failed to solve the maze
  solved_maze[x][y] = false;  // solve path doesn't includes the spot in current position
